...
const protectUniversal = async (req, res, next) => {
  ...
  if (
    ...
    try {
      ...
      // Try Partner
      let partner = await Partner.findById(decoded.id).select('-password');
      if (partner) {
        req.user = partner;
        req.userModel = 'Partner';
        // Ensure role is set for authorize middleware
      ...
      if (admin) {
        req.user = admin;
        req.userModel = 'Admin'; // or Admin model name
        return next();
      }
...
module.exports = { protect, protectAdmin, protectPartner, protectOrAdmin, protectUniversal, authorize };
